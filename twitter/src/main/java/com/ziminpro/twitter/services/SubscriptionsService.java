    ...
    public Mono<ResponseEntity<Map<String, Object>>> createSubscription(Subscription subscription) {
        return ReactiveSecurityContextHolder.getContext()
                ...
                .flatMap(auth -> {
                    ...
                    //  подписчика = текущий юзер
                    subscription.setSubscriber(UUID.fromString(userId));
    ...
    public Mono<ResponseEntity<Map<String, Object>>> deleteSubscriptionForSubscriberById(UUID subscriberId) {
        return ReactiveSecurityContextHolder.getContext()
                ...
                .flatMap(auth -> {
                    ...
                    // удалять можно только свои подписки
                    if (!userId.equals(subscriberId.toString())) {
                        response.put(Constants.CODE, "403");
                        response.put(Constants.MESSAGE, "Access Denied");
                        response.put(Constants.DATA, false);
                        return Mono.just(ResponseEntity.status(403).header(HttpHeaders.CONTENT_TYPE, Constants.APPLICATION_JSON)
                                .body(response));
                    }
